//! # cargo-ocd
//!
//! Cargo 子命令，用于通过 OpenOCD 一键烧录和调试 STM32 固件。
//!
//! ## 安装
//!
//! ```bash
...
//! 支持 STM32 全系列芯片（通过 target 和 target-triple 配置）。
...
/// 打印帮助信息
fn print_help() {
    println!("cargo-ocd — 一键编译并通过 OpenOCD 烧录/调试 STM32 固件");
    println!();
    ...
    println!("支持的芯片:   STM32 全系列（通过 target 和 target-triple 配置）");
    println!();
